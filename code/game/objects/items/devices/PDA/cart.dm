...
/obj/item/weapon/cartridge/proc/create_NanoUI_values(mob/user as mob)
	...
	if(mode==43 || mode==433)
		...
		for(var/obj/machinery/power/apc/A in L)
			apcData[++apcData.len] = list("Name" = rhtml_encode(A.area.name), "Equipment" = Status[A.equipment+1], "Lights" = Status[A.lighting+1], "Environment" = Status[A.environ+1], "CellPct" = A.cell ? round(A.cell.percent(),1) : -1, "CellStatus" = A.cell ? chg[A.charging+1] : 0)
