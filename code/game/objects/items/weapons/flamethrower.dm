...
/obj/item/weapon/flamethrower/attackby(obj/item/W as obj, mob/user as mob)
	...
	if(istype(W, /obj/item/device/analyzer) && ptank)
		...
		if(total_moles>0)
			...
			var/plasma_concentration = ptank.air_contents.plasma/total_moles
...
/obj/item/weapon/flamethrower/Topic(href,href_list[])
	...
	if(href_list["light"])
		if(!ptank)	return
		if(ptank.air_contents.plasma < 1)	return
		if(!status)	return
		lit = !lit
		if(lit)
...
/obj/item/weapon/flamethrower/proc/ignite_turf(turf/target)
	...
	new/obj/effect/decal/cleanable/liquid_fuel/flamethrower_fuel(target,air_transfer.plasma,get_dir(loc,target))
	air_transfer.plasma = 0
